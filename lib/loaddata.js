...
function loadBlogPostData(draw, top){
    ...
    d3.tsv(fileName, function(error, rawData) {
        ...
        var inputFormat = d3.timeFormat('%Y-%m-%dT%H:%M:%S');
        ...
        rawData = rawData.filter(function(d){
            ...
            var startDate =  (Date.parse('2012-12-01T00:00:00'));
            var endDate = (Date.parse('2014-01-01T00:00:00'));
            //2011 for CrooksAndLiars
            if(fileName.indexOf("Liars")>=0){
                startDate = (Date.parse('2009-12-01T00:00:00'));
                endDate = (Date.parse('2011-01-01T00:00:00'));
            }
            else if(fileName.indexOf("WikiNews")>=0){
                startDate = (Date.parse('2013-12-01T00:00:00'));
                endDate = (Date.parse('2016-01-01T00:00:00'));
            }
            else if(fileName.indexOf("Huffington")>=0){
                startDate = (Date.parse('2011-12-01T00:00:00'));
                endDate = (Date.parse('2013-01-01T00:00:00'));
            }
        ...
        var data = d3.keys(data).map(function(date, i){
            ...
            return outputFormat(Date.parse(a.date)) - outputFormat(Date.parse(b.date));
        });
        processSudden(data);

        totalData = getTop(data, topics, maxTop).slice(0,12); // omit first timestep
